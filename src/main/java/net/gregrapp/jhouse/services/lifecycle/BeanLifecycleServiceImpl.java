...
<<<<<<< HEAD
	...
	if (beans != null && beans.size() > 0)
	{
		...
		{
		    if (bean.getDriver().getDriverInterface().getTransport()
			    .isEnabled() == false)
		    {
			logger.warn(
		    ...
		    if (bean.getDriver().getDriverInterface().isEnabled() == false)
		    {
			logger.warn(
		    ...
		    if (bean.getDriver().isEnabled() == false)
		    {
			logger.warn(
				"Driver disabled, cannot instantiate device bean {}",
				bean.toString());
			continue;
		    }
		    ...
		    {
			logger.info(
				"Building bean definition for device bean {}",
				bean.toString());
			...
			{
			    logger.debug("Device is a DriverDevice, getting driver bean reference");
			    DeviceDriver driver = (DeviceDriver) appContext
				    .getBean(DRIVER_BEAN_NAME_PREFIX
					    + bean.getDriver().getId());
			...
			beanFactory.registerBeanDefinition(
				DEVICE_BEAN_NAME_PREFIX + bean.getId(),
				beanDefinition);
		    } else
		    {
			logger.info("Device bean not enabled, skipping {}",
				bean.toString());
		    }
		}
	...
	if (beans != null && beans.size() > 0)
	{
		...
		{
		    if (bean.getDriverInterface().getTransport().isEnabled() == false)
		    {
			logger.warn(
		    ...
		    if (bean.getDriverInterface().isEnabled() == false)
		    {
			logger.warn(
		    ...
		    {
			logger.info(
				"Building bean definition for device driver bean {}",
				bean.toString());
			GenericBeanDefinition beanDefinition = new GenericBeanDefinition();
			...
			if (bean.getProperties().size() > 0)
			{
			    ...
			    {
				logger.debug(
					"Setting property [{}] on bean to [{}]",
					key, bean.getProperties().get(key));
				propertyValues.add(key, bean.getProperties()
					.get(key));
			    }
			...
			beanFactory.registerBeanDefinition(
				DRIVER_BEAN_NAME_PREFIX + bean.getId(),
				beanDefinition);
		    } else
		    {
			logger.info(
				"Device driver bean not enabled, skipping {}",
				bean.toString());
		    }
		}
		beanFactory.preInstantiateSingletons();
	...
	if (beans != null && beans.size() > 0)
	{
		...
		for (InterfaceBean bean : beans)
		{
		    ...
		    {
			logger.info(
				"Building bean definition for interface bean {}",
				bean.toString());
			GenericBeanDefinition beanDefinition = new GenericBeanDefinition();
			...
			beanFactory.registerBeanDefinition(
				INTERFACE_BEAN_NAME_PREFIX + bean.getId(),
				beanDefinition);
		    } else
		    {
	...
	if (beans != null && beans.size() > 0)
	{
		...
		for (TransportBean bean : beans)
		{
		    ...
		    {
			logger.info(
				"Building bean definition for transport bean {}",
				bean.toString());
			GenericBeanDefinition beanDefinition = new GenericBeanDefinition();
			...
			beanFactory.registerBeanDefinition(
				TRANSPORT_BEAN_NAME_PREFIX + bean.getId(),
				beanDefinition);
		    } else
		    {
      ...
      if (beanFactory != null)
      {
        ...
        for (DeviceBean bean : beans)
        {
          if (bean.isEnabled())
          {
            ...
            beanDefinition.setScope(BeanDefinition.SCOPE_SINGLETON);
            logger.debug("Registering device bean definition [{}]",
                DEVICE_BEAN_NAME_PREFIX
                    + bean.getId());
            beanFactory.registerBeanDefinition(DEVICE_BEAN_NAME_PREFIX
      ...
      if (beanFactory != null)
      {
        ...
        for (DriverBean bean : beans)
        {
          if (bean.isEnabled())
          {
            ...
            beanDefinition.setScope(BeanDefinition.SCOPE_SINGLETON);
            logger.debug("Registering device driver bean definition [{}]",
                DRIVER_BEAN_NAME_PREFIX
                    + bean.getId());
            beanFactory.registerBeanDefinition(DRIVER_BEAN_NAME_PREFIX
                + bean.getId(), beanDefinition);
          }
          else
          {
